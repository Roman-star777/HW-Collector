...
export function Auction() {
  ...
  function bid() { setBids((prev) => [{ user: 'Ви', amount: prev[0].amount + 30 }, ...prev]) }
        ...
        <div className="bid-grid">
          <div className="bid-left">
            ...
            <div className="bid-list">
              {bids.slice(0, 3).map((b, i) => (<div key={i}><span>{b.user}</span><span>{b.amount} ₴</span></div>))}
            </div>
          </div>
          <div className="bid-right">
            <Button variant="primary" size="sm" onClick={bid}>Ставка {current + 30}</Button>
            <Button variant="secondary" size="sm">Авто</Button>
          </div>
        </div>
      </div>
