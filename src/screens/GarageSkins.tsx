...
export function GarageSkins() {
  ...
  return (
    <div className="pad">
      <button className="back-link" onClick={() => navigate('/garage')}><svg viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6" /></svg>Гараж</button>
      ...
      <div className="skins-grid">
        {SHELVES.map((s) => {
          ...
          return (
            <Card key={s.id} variant={active === s.id ? 'selected' : unlocked ? 'default' : 'disabled'} className="skin-card" onClick={() => unlocked && setActive(s.id)} style={{ background: s.background, borderColor: s.frameColor }}>
              <span className="skin-card__dot" style={{ background: s.glowColor }} />
              <b>{s.name}</b><span>{s.type.replace('_', ' ')}</span>
