...
import './shared.css'
import './Profile.css'
const CURRENT_LEVEL = 37; const XP_CURRENT = 2740; const XP_NEXT = 4000
const ACTIVITY = [
...
const COLLECTION_STATS = [{ label: 'Машинки', value: 78 }, { label: 'Стелажі', value: 2 }, { label: 'Рідкісні', value: 12 }, { label: 'Обмін', value: 4 }]
  ...
  return (
    <div className="pad">
      ...
      <div className="stat-row">
        <div><b>14</b><span>покупок</span></div>
        <div><b>3</b><span>виграшів</span></div>
      </div>
      <Card variant="premium" className="club-card">
        <b>Клуб колекціонерів</b><span>Отримуй ексклюзивні можливості та бонуси щомісяця!</span>
        <Button variant="gold" size="sm" style={{ marginTop: 12, width: '100%' }}>Вступити в клуб</Button>
      </Card>
      <h2 className="section-title">Моя колекція</h2>
      <div className="collection-grid">{COLLECTION_STATS.map((s) => (<div key={s.label} className="collection-tile"><b>{s.value}</b><span>{s.label}</span></div>))}</div>
      <h2 className="section-title">Моя активність</h2>
      <Card className="list-card">{ACTIVITY.map((a) => (<ListItem key={a.title} title={a.title} subtitle={a.subtitle} icon={<svg viewBox="0 0 24 24"><path d={a.icon} /></svg>} onClick={() => {}} />))}</Card>
      <h2 className="section-title">Налаштування</h2>
      <Card className="list-card">{SETTINGS.map((a) => (<ListItem key={a.title} title={a.title} subtitle={a.subtitle} icon={<svg viewBox="0 0 24 24"><path d={a.icon} /></svg>} onClick={() => {}} />))}</Card>
