...
interface ProductShotProps { shape?: keyof typeof PATHS; color: string }
export function ProductShot({ shape = 'muscle', color }: ProductShotProps) {
  const path = PATHS[shape] ?? PATHS.muscle
  const id = `shot-${gid++}`
  const light = shade(color, 70); const dark = shade(color, -60)
  return (
    <svg viewBox="0 0 128 60" className="product-shot">
      <defs>
        <linearGradient id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={light} /><stop offset=".45" stopColor={color} /><stop offset="1" stopColor={dark} /></linearGradient>
        <radialGradient id={`${id}-shadow`} cx="50%" cy="50%" r="50%"><stop offset="0" stopColor="#000" stopOpacity="0.55" /><stop offset="1" stopColor="#000" stopOpacity="0" /></radialGradient>
      </defs>
      <ellipse cx="64" cy="47" rx="46" ry="7" fill={`url(#${id}-shadow)`} />
      <g transform="translate(0,4)">
        <path d={path} fill={`url(#${id})`} /><path d={path} fill="none" stroke={light} strokeWidth="1" opacity="0.6" />
        <ellipse cx="50" cy="16" rx="28" ry="3.6" fill="#fff" opacity="0.35" />
        <circle cx="34" cy="38" r="8.5" fill="#0a0b0c" /><circle cx="34" cy="38" r="3.4" fill="#5a5d62" />
        <circle cx="96" cy="38" r="8.5" fill="#0a0b0c" /><circle cx="96" cy="38" r="3.4" fill="#5a5d62" />
      </g>
    </svg>
  )
}
