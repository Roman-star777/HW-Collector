...
- **Badge** (`src/components/Badge.tsx`) — 9 варіантів бейджів
  (new/rare/exclusive/live/sold/discount/premium/club/limited),
  розділ 06 дизайн-системи.
- **LevelBadge** — щит з зіркою, колір і назва tier-у з
  `src/data/levels.ts`. 10 tier-ів (ROOKIE…GOAT), кожен по 10
  рівнів, зі своїм акцентним кольором — готова архітектура під
  окремий пакет із 10 унікальних іконок-бейджів пізніше.
- **AuctionCard** — переюзабельна картка "ефір зараз" (LIVE-бейдж +
  таймер + CTA), розділ 13. Один компонент замість дублювання на
  кожному екрані.
- **CarCard** — додано бейдж «Новинка» та кнопку обраного (розділ 12).
